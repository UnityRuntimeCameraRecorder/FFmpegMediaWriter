...
namespace FFmpegMediaWriter
{
    ...
    internal sealed class EncodedVideoTransport
    {
        ...
        internal byte[] Wrap(byte[] data, long presentationMicroseconds)
        {
            if (!_origin.HasValue)
            {
                _origin = presentationMicroseconds;
            }
                ...
                using (var pes = new MemoryStream())
                {
                    ...
                    while (offset < payload.Length)
                    {
                        ...
                        if (adaptation > 0)
                        {
                            ...
                            if (adaptation > 1)
                            {
                                packet[5] = first ? (byte)0x10 : (byte)0;
                            }
                            if (first)
                            {
                                long clock = dts & 0x1ffffffffL;
                                packet[6] = (byte)(clock >> 25);
                                packet[7] = (byte)(clock >> 17);
                                packet[8] = (byte)(clock >> 9);
                                packet[9] = (byte)(clock >> 1);
                                packet[10] = (byte)(((clock & 1) << 7) | 0x7e);
                                packet[11] = 0;
                            }
        ...
        private static void WriteTime(Stream output, long value, int prefix)
        {
            ...
            output.WriteByte((byte)(time >> 22));
            output.WriteByte((byte)(((time >> 14) & 0xfe) | 1));
            output.WriteByte((byte)(time >> 7));
            output.WriteByte((byte)(((time << 1) & 0xfe) | 1));
        }
        private static void WriteSection(Stream output, int pid, byte[] section, ref int continuity)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                packet[5 + section.Length + i] = (byte)(crc >> (24 - i * 8));
            }
            output.Write(packet, 0, packet.Length);
        }
    }
